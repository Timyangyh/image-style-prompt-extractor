...
- 从提示词提取结果或融合结果中导入一份可编辑的提示词副本，不反写原始分析 JSON、融合 JSON 或图片分析历史。
...
- 多处修改会自动生成编号，并为每个编号保存独立修改要求，减少“标注 1 / 标注 2”互相串用。
- 参考生成模式会提交干净源图、低遮挡编号定位图和修改清单，生成 1-4 张旁路候选图，适合快速筛选和继续迭代。
- 严格 mask 保真模式仅在 OpenAI-compatible Images API 后端可用，会提交 PNG 源图和同尺寸 alpha mask；Codex OAuth、OpenAI-compatible Responses 和 OpenRouter 不支持该严格模式。
- 参考生成模式可生成本地保护范围预览和本地保护版；保护图只在本机用于后处理，不提交给 OpenRouter 或 Codex OAuth。只有 AI 输出与源图同尺寸时才会合成本地保护版。
...
1. 完成分析或融合后，把需要的提示词导入“生图工作台”。
2. 按需编辑提示词、上传参考图、选择比例和分辨率。
3. 点击开始生图。
4. 在任务卡中查看状态、预览结果、下载图片，或打开原图/生成图左右对比。
5. 需要进一步修改时，点击生成图上的“去改图”。
...
4. 选择参考生成模式，或在支持的 OpenAI-compatible Images 后端下选择严格 mask 保真模式。
5. 开始改图后，在任务卡中查看候选图、左右对比、下载结果，或把某张输出继续作为下一轮源图。
...
- OpenRouter 和 Codex OAuth 的改图路径属于参考生成，不承诺像素级源图保真。
- 本地保护版是软件后处理结果，不是模型原始输出；当 AI 输出尺寸与源图不一致时不会强行缩放贴回。
- 严格 mask 保真只适用于支持 `/images/edits` 形态的 OpenAI-compatible Images 后端。

## 系统要求

- 下载 App：M 系列 Mac，或 Windows 10 / 11 x64。
- 源码运行：M 系列 Mac、Node.js 和 npm；Windows 源码运行需要自行安装 Node.js/npm 并使用命令行启动。
- 图片分析：可用的 OpenAI-compatible 视觉模型 API。
